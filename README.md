...
## [Why software containers?](https://kubernetes.io/docs/concepts/overview/what-is-kubernetes/#why-do-i-need-kubernetes-and-what-can-it-do)
When we have an application build, we want to take this application off laptop, and get them running on the cloud or some instance in the cloud. For our case, it would be Python applications and ec2 of amazon. To achieve this, typically we have four options. 
...
Following the similar step we did for validation stage, we can containerize each stage of our pipeline and run it on some machine without installing a bunch of codes and dependencies. After deploying each of those stages as a docker container, we expect to run multiple containers. We might also need to deploy multiple containerized “workers” for each stage, which means we would have even more containers to manage. For example, we might want multiple workers to process incoming images so that this processing step is distributed. That’s why we need Kubernetes to automatically schedule and optimize our containers on our cluster. To deploy containers on Kubernetes, we will need Pachyderm be the link by telling Pachyderm about the containers via JSON specifications.

Also, we need to get the right data to the right containers at the right time. In our case, we need incoming images to be passed to a specific container and the output to be pass out to another container for each stage. Hence, we need to automate most processing steps to minimize the issues with manually running. Pachyderm also have the functionality to get the right data to the right containers and gather and version the output.
