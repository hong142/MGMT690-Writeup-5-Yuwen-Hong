...
### Create Dockerfiles
The first step to containerizing a pipeline stage with docker is to develop the application, create a dockerfile accordingly and push it to GitHub. The application script needed is exact the one you normally would develop locally and version on GitHub. You can find a lot reference for creating docker file online. What we use this time is the following 2-line code, which means add our ["validate" script](https://github.com/dwhitena/mgmt690-pipeline/blob/master/validate/validate.py) (the application) from Python. The "/code/validate" part is the location in the image we assign to the script, so that we CAN FIND IT ESILY. You can put your script anywhere in your image, and you can add as many such files as you want. The "from” at the beginning is almost always in a docker file, and it allows you to setup a base system and dependencies that someone else already created instead of recreating it each time. For official docker images, you can go to [Docker Hub](https://hub.docker.com/), a public registry of docker images. Using existing images as base image could save us a lot of time comparing to starting from scratch, and usually details about the author and the functionality is well-documented for those images. 
```
FROM python
